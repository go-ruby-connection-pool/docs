...
`go-ruby-connection-pool/connection-pool` reimplements Ruby's `connection-pool` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-connection-pool/connection-pool` directly, with no Ruby runtime;
...
Because `connection-pool` is pure Go with cgo disabled, it also compiles to **WebAssembly**
